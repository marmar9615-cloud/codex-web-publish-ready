...
export async function getWorkdirPreviewUrl(path) {
  ...
  const pending = fetch(`/api/workdir-file/sign?path=${encodeURIComponent(path)}`, {
    credentials: "same-origin",
  })
    .then(async (response) => {
      const data = await response.json().catch(() => ({}));
      if (!response.ok || !data.previewUrl) {
        throw new Error(data.error ?? `preview signing failed (${response.status})`);
      }
...
export function createUploads({ appendSystem }) {
  function renderPendingUploads() {
    ...
    host.hidden = false;
    host.innerHTML = state.pendingUploads.map((upload, index) => `
